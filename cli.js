...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 1666600000 || netId === 1666700000) {
 ...
 */
async function init({ rpc, noteNetId, currency = 'one', amount = '10' }) {
  ...
  try {
    if (web3 == undefined) {
      ...
      contractJson = await (await fetch('build/contracts/ONESmashnado.json')).json()
    ...
    if (window.web3)
      ...
      process_errormsg('Cannot find wallet. Please install wallet like as MetaMask.')   
  }  
  ...
  if (!(netId === 1666700000 || netId === 1666600000)) {
    throw new Error('You did not connect to Harmony network. Please make sure you connected to Harmony Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to Harmony Mainnet.')
  erc20 = currency !== 'one' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'one', amount: '10' }
  //await init(instance)
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    if ( parseFloat(senderBalance) < parseFloat(_amountStr) ) {
      w_display_notification('Insufficient ONE Balance in your wallet to deposit.', 'error')
      return false;
    }
    ...
    try {
      ...
      if (noteString) {
        ...
        resultStr += 'Sender account ONE balance is ' + web3.utils.fromWei(await web3.eth.getBalance(senderAccount)) +'<br/>'
